...
- **PR order:** the neutral interactions port lands first (PR1), the `ProviderSession` seam second (PR2). The seam's `Provider.create(input)` then takes a neutral `interactions` port from day one instead of Droid's handler pair, so the create path is touched once, not twice.
- **Contract grows additively.** PR2 ships only `Provider { kind, create, resume }` and `ProviderSession { provider, providerSessionId, resumeId?, stream, interrupt, close }`. `probe()` / `ProviderStatus` arrive with the registry in PR3; `ProviderCapabilities` and the lifecycle gates arrive in PR5 where a provider first needs them. Nothing speculative is merged.
- **Open questions answered:** (a) unsupported controls are hidden for non-Droid sessions; (b) the picker is an icon chip beside the model chip in the composer's existing idiom; (c) Claude and Codex models join the single existing model catalog; (d) reasoning effort uses a lossy per-provider table, unsupported values are simply absent from `supportedReasoningEfforts`; (e) Claude ships PR5 with `steer: false` (prompts queue), true mid-turn steering is a later PR; (f) Codex outside the reviewed CLI range reads as `unsupported` with a clear message.
...
- **Verification is the real CLI in the real app** for every PR from PR5 on, plus the standing gates (typecheck, lint on touched files, existing suites, file-size, boundaries, knip for renderer PRs, perf replay/compare/gates for anything on the turn path).
...
**Provider identity already exists.** `sidecar/src/providers/providerKind.ts` defines `PROVIDER_KINDS = ['droid','claude','codex']`, `DEFAULT_PROVIDER = 'droid'`, `providerKind()`, `requireProviderKind()`, `assertProviderUnchanged()`. `SessionSummary.provider: ProviderKind` is **required** (`sidecar/src/protocol.ts:117`) and mirrored at `src/types/bridge.ts:122`. `session.create` accepts `provider?: ProviderKind` (`protocol.ts:587`, `bridge.ts:604`, `src/lib/commands.ts:41`) and `SessionLifecycle.create` validates it at `SessionLifecycle.ts:139` and stamps it into the summary via `buildCreatedSessionSummary` (`sessionHelpers.ts:314,374`).

Consequences: **we use `ProviderKind`, not a new `HarnessId`.** New code lives under `sidecar/src/providers/` and `src/features/providers/`. Two gaps remain: the binding is not persisted (`history.ts:1188,1492` and `sessionAdoption.ts:203` hardcode `provider: DEFAULT_PROVIDER`), and it is not routed — `SessionLifecycle.create` calls `d.runtime.createSession(...)` unconditionally at `SessionLifecycle.ts:188`, so `provider:'claude'` silently runs Droid today.
