...
- Remote pages run sandboxed with context isolation and Node integration disabled, and cannot settle an agent request; only the main-process invoke result can. Agent-facing URLs redact OAuth codes, state, nonces, assertions, and fragments, and captures mask password and one-time-code inputs. Downloads default to a save dialog and otherwise stay inside the configured directory with sanitized names.
...
- Canonical durability uses `session-index.sqlite`; rebuildable file-summary and FTS5 state uses the separate `session-search.sqlite`. Both live in the app profile (`DROIDEX_USER_DATA_DIR`), so separate profiles have independent writer leases. Factory authentication and raw session files remain shared. The previous index under `~/.factory/droidex` is not read, copied, or deleted. An absent, old, or corrupt derived database is rebuilt from provider JSONL without modifying canonical sessions, children, or event rows; deleting `session-search.sqlite` plus its WAL/SHM files is the explicit recovery step for derived-index corruption. The worker bundle ships beside `sidecar.mjs` in packaged updates.
