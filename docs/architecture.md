...
- Sidecar Design Mode references are scoped to the current URL. It keeps the newest 32 pending references and 32 standalone screenshots; navigation, replacement, and session close delete superseded controller-owned images. A successfully written prompt pack takes permanent ownership of its referenced image paths so later cleanup cannot break an already-sent design artifact.
