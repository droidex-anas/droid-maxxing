...
- Electron owns one persistent `persist:droidex-browser` partition shared by every chat. `nativeBrowser` owns the view registry, layout, eviction (URL, viewport, and scroll only), and crash recovery; `sidecar/src/browser/*` sessions carry separate identities for routing and scope Design Mode references to the current URL, not separate cookie profiles. Clearing browser data closes views and wipes the shared partition; saved logins are deleted separately.
- Navigation is main-process gated. Agent URLs accept only external `http:`/`https:`; local files, `javascript:`/`data:`, browser-internal pages, embedded credentials, and custom schemes are rejected. The live `WebContents` URL is authoritative: `snapshot` observes without navigating, and reload falls back to the last valid page or the home page.
- `electron/browserPermissions.cjs` resolves the Settings → Browser policy. Main validates the whole `requestId + appSessionId + browserSessionId + action + autonomy` request as one transaction, “Follow autonomy” maps High/Medium/Low to full, exact-origin, and per-opening approval, and the first-party `droidex-browser` MCP server defers its generic SDK confirmation here. Camera and microphone are blocked by default with per-origin, revocable decisions; USB, HID, and serial stay denied; inspect/network/console diagnostics are off by default and never expose raw CDP.
- `electron/browserCookieImport*.cjs` owns Chrome cookie import in main. The renderer sees only an opaque single-use plan ID plus domain/count metadata, commit closes views before mutating the shared cookie store, and settings retain a secret-free receipt. Import is cookie-only: no passwords, local storage, device-bound state, or partitioned cookies, and no Safari path exists.
- `electron/browserCredentialVault.cjs` captures credentials only from a page's known main frame, encrypts them with OS-protected storage keyed by exact HTTPS origin, revalidates view/contents/origin before save or fill, and requires native approval (Touch ID where macOS offers it) per fill. Plaintext never reaches the renderer or agent.
- `electron/browserWebAuthn.cjs` owns authentication gating. Signup, sign-in, OAuth, and passkey actions need a single-use native approval; cross-origin iframe authentication fails closed; OAuth mints one short-lived sandboxed window for the exact inspected destination, off the automation bridge. Account selection cancels by default and keeps credential IDs in main; Touch ID passkeys require a Developer ID release with the `${APPLE_TEAM_ID}.app.droidex.webauthn` keychain access group.
- Remote pages run sandboxed with context isolation and Node integration disabled, and cannot settle an agent request; only the main-process invoke result can. Agent-facing URLs redact OAuth codes, state, nonces, assertions, and fragments. Snapshots, details, and captures redact sensitive editable controls, including textareas and contenteditable descendants. Downloads default to a save dialog and otherwise stay inside the configured directory with sanitized names.
- `electron/browserAgentCursor.cjs` owns the optional cursor as a click-through overlay above the page, driven by the coordinates the browser preload resolves (`electron/nativeBrowserPreload.cjs` is generated from `electron/nativeBrowserPreload/` by `npm run preload:build`); each session keeps its last point while hidden, and pages can imitate the artwork, so it is an activity indicator, not a trust signal. Preload element refs are leased to the document that produced them and invalidated on navigation.
...
- Canonical durability uses `session-index.sqlite`; rebuildable file-summary and FTS5 state uses the separate `session-search.sqlite`. Both live in `~/.factory/droidex` by default, or under `<profile>/history` for an instance launched with `DROIDEX_USER_DATA_DIR`, so separate profiles have independent writer leases. An absent, old, or corrupt derived database is rebuilt from provider JSONL without modifying canonical sessions, children, or event rows; deleting `session-search.sqlite` plus its WAL/SHM files is the explicit recovery step for derived-index corruption. The worker bundle ships beside `sidecar.mjs` in packaged updates.
