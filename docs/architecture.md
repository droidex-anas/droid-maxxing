...
- Browser element refs are leased to the exact isolated document snapshot that produced them. Navigation invalidates the lease and cancels any pointer action still waiting on cursor motion, so a compacted or delayed turn cannot apply an old ref to replacement content. Click, hover, and scroll run in the isolated preload after the native cursor reaches the validated live point; this keeps background browser work focus-neutral. Scrolling resolves the current nearest scrollable ancestor instead of replaying a cached box, waits for layout settlement, and reports movement versus a reached boundary with the fresh snapshot.
