...
test('recent histories index first while old histories wait for idle slices', async () => {
...
test('a transient file read failure stays queued for a later slice', async () => {
...
test('one unreadable provider does not delay a healthy provider in the same lane', async () => {
...
test('a newly changed recent chat preempts a pending archive timer', async () => {
...
test('an in-flight old slice cannot overwrite a newer watcher entry', async () => {
...
test('a stable truncated tail parks until a watcher reports new bytes', async () => {
...
test('a full reconcile cancels a deleted file in flight before stale rows commit', async () => {
