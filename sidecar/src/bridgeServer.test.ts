...
test('commands received during async admission drain in order after the snapshot', async () => {
  ...
  await withServer(
    async (harness) => {
      const received: string[] = [];
      const socket = new WebSocket(
        `ws://127.0.0.1:${String(harness.port)}?token=${harness.token}&bridgeProtocol=${String(BRIDGE_PROTOCOL_VERSION)}&resumeGeneration=old-generation&resumeSeq=0`,
      );
      socket.on('message', (raw) => received.push(String(raw)));
      ...
      releaseSnapshot?.(runtimeSnapshot());
...
test('shutdown discards commands queued during async admission', async () => {
  ...
  await withServer(
    ...
    () => {
      markSnapshotStarted?.();
      return new Promise<BridgeRuntimeSnapshot>(() => undefined);
    },
  );
});
...
function socketRoundTrip(socket: WebSocket, timeoutMs = 2_000): Promise<void> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('socket round trip timed out')), timeoutMs);
    socket.once('pong', () => {
      clearTimeout(timer);
      resolve();
    });
...
function runtimeSnapshot(): BridgeRuntimeSnapshot {
  return {
