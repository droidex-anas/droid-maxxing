...
import { assertModelSelection, clip } from './automationInput.js';
import {
  auditRunSelection,
  failInterruptedRuns,
  newQueuedRun,
...
export class AutomationRuns {
  ...
  private async startRun(run: AutomationRun): Promise<void> {
    ...
    run.startedAt = now;
    run.finishedAt = null;
    run.clientRef = `automation:${run.id}:${randomUUID()}`;
    run.appSessionId = null;
    run.resolvedCwd = null;
    run.error = null;
    this.projectRun(run, 'starting', now);
    try {
      // Recording 'starting' can fail, and a run left starting in memory would
      // block the queue forever, so the write is settled through the same path
      // as a failed launch.
      ...
      // Preparing a worktree takes real time, so this run may already be settled
      // or gone. A workspace must never be attached to a run nothing tracks, and
      // the one just created has to go with it.
      ...
      this.timers.armSessionCreate(current.id);
      await this.options.launchSession(sessionCommandForRun(current));
    } catch (error) {
      await this.finish(run.id, 'failed', errorMessage(error));
    }
  }
  ...
  private observeActivity(appSessionId: string): void {
    ...
    if (run?.status === 'running') this.streamingSeen.add(run.id);
  }
