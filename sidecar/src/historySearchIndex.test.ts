...
test('indexed search survives raw-file removal and preserves aliases and substring behavior', async () => {
...
test('canonical aliases refresh only when the summary identity revision advances', async () => {
...
test('one high-volume provider cannot starve another matching session', async () => {
...
test('overlapping index connections commit each source event exactly once', async () => {
  ...
  try {
    ...
    assert.equal(
      firstDb
        .prepare('SELECT count(*) AS count FROM history_search_rows WHERE provider_session_id = ?')
...
test('reconciliation indexes only changed files and removes deleted sessions', async () => {
...
test('a same-size rewrite with a new session revision rebuilds indexed content', async () => {
...
test('an appended session preserves indexed rows and adds only the new tail', async () => {
...
test('partial indexing resumes from its committed byte cursor after restart', async () => {
...
test('invalid persisted byte cursors are discarded and rebuilt safely', async () => {
...
test('reconciliation indexes messages before the newest five megabytes', async () => {
...
test('stale reconciliation and search stop without publishing results', async () => {
...
test('a corrupt derived search schema rebuilds without touching canonical history rows', async () => {
